...
const ExamRadio = ({
  ...
  // 1. Get the student's answer and feedback for this specific question
  const submittedAnswerObj = submission?.answers.find(
    (a) => a.questionId === q._id,
  );
  const studentAnswer = submittedAnswerObj?.answer;
  const feedback = submittedAnswerObj?.feedback;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 5. AI Feedback for Wrong Answer */}
      {submission && feedback && (
        <div className="mt-4 p-4 rounded-lg bg-indigo-50 border border-indigo-200 text-indigo-900 dark:bg-indigo-950/30 dark:border-indigo-900/50 dark:text-indigo-200 flex gap-3 shadow-sm">
          <Sparkles className="w-5 h-5 text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" />
          <div>
            <h4 className="font-semibold text-sm mb-1 text-indigo-800 dark:text-indigo-300">
              AI Explanation
            </h4>
