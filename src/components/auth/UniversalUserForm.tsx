...
const UniversalUserForm = ({ type, initialData, onSuccess, role }: Props) => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const { data } = (await api.get("/users?role=student&limit=1000")) as {
          data: { users: user[]; pagination: pagination };
        };
        setStudents(data.users);
      } catch (error) {
        if (type !== "login" && role === "parent") {
  ...
  useEffect(() => {
    if (initialData && isUpdate) {
      ...
      form.reset({
        ...
        subjectIds: initialData.teacherSubject?.map((s: any) => s._id) || [],
        childrenIds: initialData.children?.map((c: any) => c._id) || [],
      });
    }
  ...
  async function onSubmit(data: FormValues) {
    try {
      ...
      const payload = {
        ...data,
        studentClass: data.classId || null,
        teacherSubject: data.subjectIds && data.subjectIds.length > 0 ? data.subjectIds : [],
        children: data.childrenIds && data.childrenIds.length > 0 ? data.childrenIds : [],
      };
      if (isLogin) {
  ...
  const showChildrenSelector = !isLogin && role === "parent";
          ...
          <div className="col-span-2 space-y-2">
            ...
            {showChildrenSelector && (
              <CustomMultiSelect
                control={form.control}
                name="childrenIds"
                label="Assigned Students (Children)"
